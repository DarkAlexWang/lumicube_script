...
CONFIG = {
    ...
    "alerts": {
        ...
        "min_indoor_temp_c": 18.0,
        "max_indoor_temp_c": 31.0,
        "min_humidity": 25.0,
        "max_humidity": 80.0,
...
def read_attr(module_name, attr_name):
    try:
        if module_name == "env_sensor":
            if attr_name == "temperature":
                return env_sensor.temperature
            if attr_name == "humidity":
                return env_sensor.humidity
            if attr_name == "pressure":
                return env_sensor.pressure
        elif module_name == "light_sensor":
            if attr_name == "ambient_light":
                return light_sensor.ambient_light
            if attr_name == "red":
                return light_sensor.red
            if attr_name == "green":
                return light_sensor.green
            if attr_name == "blue":
                return light_sensor.blue
        elif module_name == "pi":
...
def update_indoor():
    STATE["indoor"].update({
        "temp_c": read_attr("env_sensor", "temperature"),
        "humidity": read_attr("env_sensor", "humidity"),
        "light": read_attr("light_sensor", "ambient_light"),
        "last_update": time.time(),
    })
...
def active_critical_alerts():
    ...
    indoor = STATE["indoor"]
    status = STATE["status"]
    if indoor["temp_c"] is not None and indoor["temp_c"] > a["max_indoor_temp_c"]:
        alerts.append(f"Indoor hot {indoor['temp_c']:.1f}C")
    if indoor["temp_c"] is not None and indoor["temp_c"] < a["min_indoor_temp_c"]:
        alerts.append(f"Indoor cold {indoor['temp_c']:.1f}C")
    if indoor["humidity"] is not None and indoor["humidity"] > a["max_humidity"]:
        alerts.append(f"Humidity high {indoor['humidity']:.0f}%")
    if indoor["humidity"] is not None and indoor["humidity"] < a["min_humidity"]:
        alerts.append(f"Humidity low {indoor['humidity']:.0f}%")
...
def render_indoor_page():
    indoor = STATE["indoor"]
    comfort = comfort_label(indoor["temp_c"], indoor["humidity"])
    accent = GREEN if comfort == "Comfortable" else ORANGE
    draw_centered_text("INDOOR", [
        f"Temp  {fmt_temp(indoor['temp_c'])}",
        f"Hum   {fmt_pct(indoor['humidity'])}",
        f"Light {fmt_pct(indoor['light'])}",
        comfort,
    ], accent=accent)
